...
public class BoardDao {
	...
	public BoardDao(){
		try{
				...
				ds = (DataSource)ctx.lookup("java:comp/env/jdbc/oracleDB");  //java:comp/env/ 가상의 프로토콜임 그냥쓰면됨
		}
		catch(Exception err){
	...
	// 답변 달기
	public void replyUpdatePos(BoardDto dto)//부모글에 대한 매개변수
	{
		try
		{
			String sql = "update tblboard set pos=pos+1 where pos>?";  // ? 는 변수로 부모글의 pos값임
			con = ds.getConnection();
			stmt = con.prepareStatement(sql);
	...
	public void replyBoard(BoardDto dto)  //실제저장하는 답변글에 대한 매개변수
	{
		try
		{
			String sql = "insert into tblBoard(num, name, email, homepage,"
